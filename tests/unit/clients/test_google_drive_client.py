...
from tentaclio_gdrive.clients import GoogleDriveFSClient
from tentaclio_gdrive.clients.google_drive_client import (
    _CreateRequest,
...
def test_load_credentials(mocker, token_file):
    mocked_creds = mocker.patch("tentaclio_gdrive.clients.google_drive_client.Credentials")
...
class TestGoogleDriveFSClient:
    ...
    @pytest.mark.parametrize(
        "url",
        (
            "gdrive:///My Drive/folder/",
            "gdrive:///My Drive/folder",
        ),
    )
    def test_scandir_folder(self, mocker, folder_descriptor, url):
        lister = mocker.patch("tentaclio_gdrive.clients.google_drive_client._ListFilesRequest")
    ...
    def test_download(self, mocker, client):
        ...
        media_download = mocker.patch(
            "tentaclio_gdrive.clients.google_drive_client.MediaIoBaseDownload"
        )
...
class TestListDrivesRequest:
    def test_yielder(self, mocker, drive_props, file_descriptor):
        mocked_get_drive_root = mocker.patch(
            "tentaclio_gdrive.clients.google_drive_client._get_drive_root"
        )
...
def test_get_drive_root_parent_not_found(mocker):
    mock_random_parent = mocker.patch(
        "tentaclio_gdrive.clients.google_drive_client._get_random_parent"
    )
...
def test_home_variable_set(mocker):
...
def test_home_variable_set_windows(mocker):
    """Test DEFAULT_TOKEN_FILE is correct."""
...
def test_home_variable_set_no_home(mocker):
    """Test DEFAULT_TOKEN_FILE is correct."""
